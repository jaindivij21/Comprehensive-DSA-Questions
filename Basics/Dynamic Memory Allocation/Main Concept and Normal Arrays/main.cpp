// Program to describe Dynamic Memory Allocation
...
int main()
{
    ...
    int b[100] = {0};
    cout << sizeof(b) << endl; // prints the size of entire array
    ...
    // Makes use of heap memory (larger) instead of the smaller stack memory
    // pointers are made in stack memory which point to the larger memory allocations like arrays in the heap memory

    int *x = new int; // dynamic basket that stores an int
    // *NEW* keyword is used for memory allocation
    // *DELETE* keyword is used for memory de-allocation

    // remember to reassign dynamic memory portion to another pointer,
