...
bool solveNQueen(int board[][10], int i, int n) {
    ...
    if (i == n) {
        ...
        cout << endl;
        count++;
        // we could return true, but we return false to print other configurations also
        return false;
    }
    ...
    for (int j = 0; j < n; j++) {
        ...
        if (isSafe(board, i, j, n)) {
            ...
            if (nextQueenPossible)   // if next queens placement is possible
                return true;
            else {
                //  if not possible, our assumption is wrong
                board[i][j] = 0;    // BACKTRACK
            }
        }
    }
    // if none of the items (values of j) fit, return false
    return false;
}
